...
describe('buildInitializer', () => {
  ...
  test('should build an initialization module', async () => {
    ...
    expect(content).toMatchInlineSnapshot(`
     ...
     const batchesDisposers = [];

     async function $dispose() {
       for(const batchDisposers of batchesDisposers.reverse()) {
         await Promise.all(
           batchDisposers
     ...
     // The service "$ready" is a managed service
     ...
     export async function initialize(services = {}) {
       ...
       const batch1 = {
         ...
         dep2: initDep2({
           ...
           if(provider.dispose) {
             batchesDisposers[1].push(provider.dispose);
           }
  ...
  test('should work with simple internal services dependencies', async () => {
    ...
    expect(content).toMatchInlineSnapshot(`
     ...
     // The service "$ready" is a managed service
     const NODE_ENV = "development";
     // The service "$fatalError" is a managed service
     // The service "$dispose" is a managed service
     // The service "$instance" is a managed service
